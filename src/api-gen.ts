		...
		for (const versionKey in client) {
			...
			for (const groupKey in groups) {
				...
				for (const commandKey in groups[groupKey]) {
					...
					const args = [];
					if (command.args !== "undefined" && command.args) args.push(`args: ${command.args}`);
					const fileArgs = [];
					if (command.files) {
			...
			api += "}\n\n";
			api += `export function api${clientKey.charAt(0).toUpperCase() + clientKey.slice(1)}${versionKey}Factory(fetcher:(cmd:string, args?:any, files?: any)=>Promise<any>):API_${clientKey.toUpperCase()}_${versionKey}{\n`;
			api += "\t return {\n";
			for (const groupKey in groups) {
				api += `\t\t${groupKey}:{\n`;
				for (const commandKey in groups[groupKey]) {
					const command = groups[groupKey][commandKey];
					let method = "";
					if (command.args === "undefined" || !command.args && command.files === "") method = `()=>fetcher('${groupKey}.${commandKey}')`;
					if (command.args !== "undefined" && command.args && command.files !== "") method = `(args:any, files?: Record<string, string[]>)=>fetcher('${groupKey}.${commandKey}', args, files)`;
					if (command.args === "undefined" || !command.args && command.files !== "") method = `(files?: Record<string, string[]>)=>fetcher('${groupKey}.${commandKey}', undefined, files)`;
					if (command.args !== "undefined" && command.args && command.files === "") method = `(args:any)=>fetcher('${groupKey}.${commandKey}', args)`;
