		...
		for (const versionKey in client) {
			...
			for (const groupKey in groups) {
				...
				for (const commandKey in groups[groupKey]) {
					...
					const args = [];
					if (command.args) args.push(`args: ${command.args}`);
					const fileArgs = [];
					if (command.files) {
			...
			for (const groupKey in groups) {
				...
				for (const commandKey in groups[groupKey]) {
					const command = groups[groupKey][commandKey];
					let method = "";
					if (command.args === "" && command.files === "") method = "()=>fetcher()";
					if (command.args !== "" && command.files !== "") method = "(args:any, files?: Record<string, string[]>)=>fetcher(args, files)";
					if (command.args === "" && command.files !== "") method = "(files?: Record<string, string[]>)=>fetcher(undefined, files)";
					if (command.args !== "" && command.files === "") method = "(args:any)=>fetcher(args)";
